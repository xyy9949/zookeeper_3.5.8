...
public class Follower extends Learner{
    ...
    public String getFollowerStateString(){
        LinkedBlockingQueue<Request> pendingTxns = fzk.pendingTxns;
        ConcurrentLinkedQueue<Request> pendingSyncs = fzk.pendingSyncs;
        StringBuffer pendingTxnsString = new StringBuffer();
        pendingTxnsString.append("[");
        for(Request request : pendingTxns)
        {
            pendingTxnsString.append(request.toStringNew()).append(",");
        }
        if(pendingTxnsString.charAt(pendingTxnsString.length()-1) == ',')
            pendingTxnsString.deleteCharAt(pendingTxnsString.length()-1);//delete the last comma
        pendingTxnsString.append("]");

        StringBuffer pendingSyncsString = new StringBuffer();
        pendingSyncsString.append("[");
        for(Request request : pendingSyncs)
        {
            pendingSyncsString.append(request.toStringNew()).append(",");
        }
        ...
        pendingSyncsString.append("]");
        return "lastQueued: " + lastQueued + ", pendingSyncs:" + pendingSyncsString + ", pendingTxns:" + pendingTxnsString;
    }
}
