...
import org.apache.zookeeper.server.util.StateCollectingUtil;
import org.apache.zookeeper.server.util.TypeSelectingUtil;
...
public class CommitProcessor extends ZooKeeperCriticalThread implements
    ...
    //HK  add param leader
    public void commit(Leader leader, Request request) {
        if (stopped || request == null) {
            return;
        }
        if (LOG.isDebugEnabled()) {
        ...
        if (!isProcessingCommit()) {
            ...
            if(TypeSelectingUtil.isContained(request.type)) {
                StateCollectingUtil stateCollectingUtil = new StateCollectingUtil(leader.self.getId(), "COMMIT", request.zxid, "null");
                stateCollectingUtil.generateStateContextCommit(leader);
    ...
    //HK  add param follower
    public void commit(Follower follower, Request request) {
        if (stopped || request == null) {
            return;
        }
        if (LOG.isDebugEnabled()) {
        ...
        if (!isProcessingCommit()) {
            //HK
            if(TypeSelectingUtil.isContained(request.type)) {
                StateCollectingUtil stateCollectingUtil = new StateCollectingUtil(follower.self.getId(), "COMMIT", request.zxid, "null");
                stateCollectingUtil.generateStateContextCommit(follower);
                stateCollectingUtil.writeToFile();
            }
